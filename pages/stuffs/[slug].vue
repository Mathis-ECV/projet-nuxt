...
/* Gere le filtre des stuffs  en fonction du side (par default c'est les stuff T d'affiché)*/
const currentFilter = ref('t');

const filteredStuffs = computed(() => {
  if (currentFilter.value === null) {
    return AllStuffs.value[getStuffPropertyName()];
  } else {
    return AllStuffs.value[getStuffPropertyName()].filter(stuff => stuff.stuffSide === currentFilter.value);
  }
});
...
/* Fin Gere le filtre des stuffs  en fonction du side */
</script>
  ...
  <section class="container">
    ...
    <div v-for="map in AllMaps_data.allMaps" :key="map.id">
      <div v-if="slug === map.mapSlug">
        ...
        <button @click="setFilter('t')" :class="{ active: currentFilter === 't' }">T</button>
        <button @click="setFilter('ct')" :class="{ active: currentFilter === 'ct' }">CT</button>
          ...
          <div class="map-top-view" :style="{ 'background-image': `url(${map.mapTopView.url})` }">
            <div v-for="stuffs in filteredStuffs" :key="stuffs.id">
              <div v-if="stuffs.stuffLandingPositionX && stuffs.stuffLandingPositionY">
                <div class="pin pin-landing" ref="pin-landing" :class="{ ['id-' + stuffs.id]: true }"
...
export default {
  data() {
    return {
      ...
      currentFilter: 't' // Par défaut, 'T' est sélectionné
    };
  },
  methods: {
